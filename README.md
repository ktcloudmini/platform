...
이 문서는 `main` 브랜치 기준으로, 현재 구현된 CI/CD 구조를 팀원에게 공유하기 위한 설명서입니다.
...
현재 목표 구조는 runner 설치/등록을 **UserData(인프라)로 위임**하는 것입니다.따라서 GitHub-hosted runner가 Monitoring에 SSH로 접근해 runner를 설치하는 흐름이 남아있다면 제거/비활성화해야 합니다.
